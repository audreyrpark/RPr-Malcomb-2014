...
author: "Joseph Holler, Kufre Udoh, Drew An-Pham, Audrey Park, Middlebury Open GIScience Classes"
date: "`r Sys.Date()`"
...
```{r reproduction-figure-4-map, message = F, eval = FALSE}
tmap_mode("plot")
...
$$
VulnerabilityM = (40 - Adaptive Capcity) * Livelihood Sensitivity * (Drought Exposure + Flood Risk)
$$
```{r combined-vulnerability-M}
# final output (adding component rasters)
vulnerability_r_M <- (40 - capacity_r) * sensitivity_r * (drought_r + flood_r) 
# 100 = highest vulnerability, 0 = lowest vulnerability
names(vulnerability_r_M) <- "vulnerabilityM"
plot(vulnerability_r_M)
```

```{r find break & limit values for vulnerability map-M}
vuln_max_M <- max(vulnerability_r_M[[1]],na.rm = TRUE)
vuln_min_M <- min(vulnerability_r_M[[1]],na.rm = TRUE)
```


### Figure 5 Multiplicative
```{r vulnerability-map-M}
vuln_map_M = ggplot() +
  geom_stars(data = vulnerability_r_M) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(vuln_min_M, vuln_max_M),
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(vuln_min_M, vuln_max_M)
  ) +
  labs(title = "Malawi Vulnerability to Climate Change with Multiplicative Score") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
# Discussion
The renanalysis of the Malcomb et al study changed the vulnerability model from an additive to a multiplicative one. Instead of calculating vulnerability as a product of (40-AdaptiveCapacity) + LivelihoodSensitivy + DroughtExposure + FloodRisk, the reanalysis calculates and visualizes a vulnerability score that is the product of (40-Adaptive Capacity) * LivelhoodSensitivity * (DroughtExposure + Flood Risk). This change created a higher selectivity in visualizing areas of Malawi that are most vulnerable. While the additive model has vulnerability values ranging from around 40 to 80, the multiplicative model ranges from 0 to around 15000. 

# Conclusions
The reanalysis suggests that using both additive and multiplicative scores may be useful in evaluating the vulnerability of a region. The additive method does not highlight the most vulnerable regions like the multiplicative score does, but it also has the advantage of not amounting to zero if one of the values is zero. Creating a vulnerability score is an undeniably objective task, requiring the transformation of qualitative data to a quantitative formula. Therefore, investigating outcomes through more than just one model can be useful in increasing the robustness of a vulnerability model.
